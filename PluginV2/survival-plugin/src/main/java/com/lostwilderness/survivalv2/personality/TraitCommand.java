...
public class TraitCommand implements CommandExecutor {
    ...
    private void sendHelp(CommandSender sender) {
        ...
        if (sender.isOp()) {
            sender.sendMessage("§e/trait temple <element> §7- Mark temple complete (Admin)");
            sender.sendMessage("§e/trait set <trait> §7- Force trait assignment (Admin)");
            sender.sendMessage("§e/trait quiz §7- Reset quiz (Admin)");
        }
        sender.sendMessage("§8§m------------------------------------------------");
    }
