...
- **Config:** `YamlEvents.get(plugin)` → `events.blood_moon.*` (enabled, chance_per_night, min_day, max_times_per_season, world_time_night, freeze_night).
...
- **EventContext:** Not used.
- **Pattern:** **Bed leave = buff, damage = conditional remove**; PlayerBedEnterEvent only sends message; EntityDamageEvent + runTaskLater to strip buff when health ≤ 10.
...
- **What it does:** In Nether, WitherSkeleton spawn: chance to buff (tag, speed, 2× health/attack, glowing, name); chance to spawn extra WitherSkeleton with chunk cooldown; on death by player, chance for extra loot and small chance for skull.
